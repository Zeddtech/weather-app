...
function getLocation() {
  if (navigator.geolocation) {
    ...
    alert(
      " unable to get your geolocation information, your browser does not support geolocation! to get your current position weather information click on the search icon then search for your city name "
    );
...
function errorHandler(err) {
  if (err.code == 1) {
    alert(
      "Error: Unable to get your geolocation information  because the page didn't have the permission to do it. please turn on your device  location and grant acess to location to get your weather information or You can  search city names to get it's current weather information"
    );
  } else if (err.code == 2) {
...
function render(cityname, state, country, weatherInfo) {
  ...
  weatherInfo.daily.forEach((el, indx) => {
    if (indx != 0) {
      ...
      <p class="temp" title='wind speed'>${f(
        el.wind_speed
      )}<small>m/s</small></p>
      </div>`;
    }
